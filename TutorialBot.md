...
`setup` is useful for running one time code that creates configures Arduino pins so we can do things like add an LED or attach a Servo motor. For example, if we attach the positive side of an LED to the Arduino
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk5NTc2MzI4NCw5NTMwNjA3NzMsMTk0Mz
